...
select `u`.`id` AS `user_id`,coalesce((select sum(`brewme`.`donations`.`amount`) from `brewme`.`donations` where `brewme`.`donations`.`user_id` = `u`.`id` and `brewme`.`donations`.`status` = 'succeeded'),0) + coalesce((select sum(`brewme`.`memberships`.`amount`) from `brewme`.`memberships` where `brewme`.`memberships`.`user_id` = `u`.`id` and `brewme`.`memberships`.`status` = 'active'),0) AS `total_earned`,coalesce((select sum(`brewme`.`payouts`.`amount`) from `brewme`.`payouts` where `brewme`.`payouts`.`user_id` = `u`.`id` and `brewme`.`payouts`.`status` = 'completed'),0) AS `total_paid_out`,(select count(0) from `brewme`.`supporters` where `brewme`.`supporters`.`user_id` = `u`.`id`) AS `supporter_count` from `brewme`.`users` `u`;
...
select `d`.`user_id` AS `user_id`,'coffee' AS `support_type`,coalesce(`d`.`display_name`,'Anonymous') AS `display_name`,`d`.`message` AS `message`,`d`.`cups` AS `cups`,`d`.`amount` AS `amount`,`d`.`reply_message` is not null AS `replied`,`d`.`created_at` AS `created_at` from `brewme`.`donations` `d` where `d`.`status` = 'succeeded' union all select `m`.`user_id` AS `user_id`,'membership' AS `support_type`,coalesce(`m`.`display_name`,'Anonymous') AS `display_name`,NULL AS `message`,0 AS `cups`,`m`.`amount` AS `amount`,0 AS `replied`,`m`.`started_at` AS `created_at` from `brewme`.`memberships` `m`;
